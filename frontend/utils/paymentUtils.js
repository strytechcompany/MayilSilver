...
export const buildSummary = (cash, weight, gstSettings) => {
  ...
  const gstPercent = toNum(gstSettings?.gstPercentage, 3);

  // Original Silver Rate — never recalculated from the taxable amount.
  const rate    = wt > 0 ? totalAmount / wt : 0;
  const taxable = gstPercent > 0 ? totalAmount / (1 + gstPercent / 100) : totalAmount;
  const taxableWeight = rate > 0 ? taxable / rate : wt;
